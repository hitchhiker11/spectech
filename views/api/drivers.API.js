...
if(document.forms.addDriver){
    ...
    const detailDivShow = ({ accidentRecord, bateOfBirth, convictedFelony, crime, curAddress, currentlyEmp, dateAdded, driverFullName, driverIdNo, driverPhoto, drivingLicense, email, emergence, expBus, expTaxi, expTractor, expTrailer, expTruck, expVan, fullKnowledge, highestEdu, idNumber, model, phoneHome, phoneMobile, position, prevAddress, prevCity, prevRegion, preventedLawful, regisNumber, salary, traffic, workedInCompany }) => {
        ...
        return `
        <h6>Фото</h6> <a href="/uploads/drivers/${driverPhoto}" target="_blank"><img src="/uploads/drivers/${driverPhoto}" alt="user-avatar" class="d-block rounded" height="100" width="100" id="uploadedAvatar" /></a> <br>
        <h6>Имя</h6> <p>${driverFullName}</p> <br>
        <h6>Дата регистрации водителя</h6> <p>${dateAdded}</p> <br>
        <h6>Идентификационный номер водителя</h6> <p>${driverIdNo}</p> <br>
        <h6>Дата рождения</h6> <p>${bateOfBirth}</p> <br>
        <h6>Номер телефона (домашний)</h6> <p>${phoneHome}</p> <br>
        <h6>Номер телефона (мобильный)</h6> <p>${phoneMobile}</p> <br>
        <h6>Номер удостоверения личности</h6> <p>${idNumber}</p> <br>
        <h6>Электронная почта</h6> <p>${email}</p>
        <h6>Заявленная должность</h6> <p>${position}</p>
        <h6>Заработная плата</h6> <p>${salary}</p><hr />
        <h6>Адрес текущего места жительства</h6> <p>${curAddresses[0] ? curAddresses[0] : '-'}</p>
        <h6>С</h6> <p>${curAddresses[1] ? curAddresses[1] : '-'}</p>
        <h6>По</h6> <p>${curAddresses[2] ? curAddresses[2] : '-'}</p> <hr>

        <h6>Предыдущие адреса водителя</h6> <br>
        ${prevAddresses ? prevAddresses.map((prevAddress, index) => {return `<h6>Предыдущий адрес ${index+1}</h6> <p>${prevAddress}</p> <br><h6>С ${index+1}</h6> <p>${prevFroms[index]}</p> <br><h6>По ${index+1}</h6> <p>${prevTos[index]}</p> <hr>`}) : 'Нет предыдущих адресов <hr>'}

        <h6>В случае чрезвычайной ситуации, уведомить:</h6><br />
        <h6>Имя</h6> <p>${emergences[0] ? emergences[0] : '-'}</p> <br>
        <h6>Контактный номер</h6> <p>${emergences[1] ? emergences[1] : '-'}</p> <hr>

        <h6>Предыдущее места работы водителя</h6>
        ${workedInCompanys ? `<h6>Да</h6> <br /> <h6>С</h6> <p>${workedInCompanys[0] ? workedInCompanys[0] : '-'}</p> <hr>
        <h6>По</h6> <p>${workedInCompanys[1] ? workedInCompanys[1] : '-'}</p> <hr>` : 'Нет предыдущих мест работы <hr />'}

        <h6>Запрещено ли водителю законное трудоустройство в качестве водителя?</h6> <p>${preventedLawful}</p> <hr />

        <h6>Был ли водитель осужден за уголовное преступление, правонарушение или криминальное нарушение?</h6> <p>${convictedFelony}</p> <hr />
        <h6>Имеет ли водитель полное знание Федеральных правил безопасности на дорогах?</h6> <p>${fullKnowledge}</p> <hr />

        <h4>Информация о водительском удостоверении  </h4><br>
        <h6>Номер водительского удостоверения</h6> <p>${drivingLicenses[0] ? drivingLicenses[0] : '-'}</p> <br>
        <h6>Город/городок</h6> <p>${drivingLicenses[1] ? drivingLicenses[1] : '-'}</p> <br>
        <h6>Срок действия</h6> <p>${drivingLicenses[2] ? drivingLicenses[2] : '-'}</p> <br>
        <h6>Тип удостоверения</h6> <p>${drivingLicenses[3] ? drivingLicenses[3] : '-'}</p> <hr>

        <h4>Образование</h4><br>
        <h6>Высшее образование водителя</h6> <p>${highestEdus[0] ? highestEdus[0] : '-'}</p> <br>
        <h6>Завершено</h6> <p>${highestEdus[1] ? highestEdus[1] : '-'}</p> <hr>

        <h4>Опыт вождения</h4><br>
        <h6>Трактор</h6>
        ${expTractors ? `<h6>Тип оборудования</h6> <p>${expTractors[0] ? expTractors[0] : '-'}</p>
        <h6>Количество лет</h6> <p>${expTractors[1] ? expTractors[1] : '-'}</p> <br> <h6>Штаты, в которых водил</h6> <p>${expTractors[2] ? expTractors[2] : '-'}</p>` : 'Нет опыта на тракторах'} <br> <br>

        <h6>Грузовик</h6>
        ${expTrucks ? `<h6>Тип оборудования</h6> <p>${expTrucks[0] ? expTrucks[0] : '-'}</p>
        <h6>Количество лет</h6> <p>${expTrucks[1] ? expTrucks[1] : '-'}</p> <br> <h6>Штаты, в которых водил</h6> <p>${expTrucks[2] ? expTrucks[2] : '-'}</p>` : 'Нет опыта на грузовиках'} <br> <br>

        <h6>Прицеп</h6>
        ${expTrailers ? `<h6>Тип оборудования</h6> <p>${expTrailers[0] ? expTrailers[0] : '-'}</p>
        <h6>Количество лет</h6> <p>${expTrailers[1] ? expTrailers[1] : '-'}</p> <br> <h6>Штаты, в которых водил</h6> <p>${expTrailers[2] ? expTrailers[2] : '-'}</p> ` : 'Нет опыта на прицепах'} <br> <br>

        <h6>Автобус</h6>
        ${expBuss ? `<h6>Тип оборудования</h6> <p>${expBuss[0] ? expBuss[0] : '-'}</p>
        <h6>Количество лет</h6> <p>${expBuss[1] ? expBuss[1] : '-'}</p> <br> <h6>Штаты, в которых водил</h6> <p>${expBuss[2] ? expBuss[2] : '-'}</p>` : 'Нет опыта на автобусах'} <br> <br>

        <h6>Фургон</h6>
        ${expVans ? `<h6>Тип оборудования</h6> <p>${expVans[0] ? expVans[0] : '-'}</p>
        <h6>Количество лет</h6> <p>${expVans[1] ? expVans[1] : '-'}</p> <br> <h6>Штаты, в которых водил</h6> <p>${expVans[2] ? expVans[2] : '-'}</p>` : 'Нет опыта на фургонах'} <br> <br>

        <h6>Такси</h6>
        ${expTaxis ? `<h6>Тип оборудования</h6> <p>${expTaxis[0] ? expTaxis[0] : '-'}</p>
        <h6>Количество лет</h6> <p>${expTaxis[1] ? expTaxis[1] : '-'}</p> <br> <h6>Штаты, в которых водил</h6> <p>${expTaxis[2] ? expTaxis[2] : '-'}</p>` : 'Нет опыта на такси'} <br> <br>
        <hr />

        <h4>Запись о происшествиях за последние 3 года</h4>
        ${accidentRecords ? `<h6>Дата происшествия</h6> <p>${accidentRecords[0] ? accidentRecords[0] : '-'}</p>
        <h6>Характер происшествия</h6> <p>${accidentRecords[2] ? accidentRecords[2] : '-'}</p> <br> <h6>травмы?</h6> <p>${accidentRecords[3] ? accidentRecords[3] : '-'}</p> <br>` : 'Нет'} <hr />
    
        <h4>проблемы с нарушениями ПДД за последние 3 года, не связанные с парковкой
        </h4>
        ${traffics ? `<h6>Дата нарушения</h6> <p>${traffics[0] ? traffics[0] : '-'}</p>
        <h6>Нарушение</h6> <p>${traffics[1] ? traffics[1] : '-'}</p> <br> <h6>Регион</h6> <p>${traffics[2] ? traffics[2] : '-'}</p> <br> <h6>Итог?</h6> <p>${traffics[3] ? traffics[3] : '-'}</p> <br>` : 'Нет'} <hr />
    `;
    }
    ...
    const tableTr = (driver) => {
        ...
        <tr>
            ...
            <td>${driver.email}</td>
            <td>${driver.model ? driver.model : 'Пока нет автомобиля'}</td>
    ...
    const loadedBtn = `Добавить водителя`;
        ...
        if(drivers.message && drivers.message == "Redirect"){
            ...
            if(page === 1){
                ...
                <li class="page-item prev">
                    <a class a page-link href="javascript:void(0);"><i class="tf-icon bx bx-chevron-left"></i></a>
                </li>
              `;
            }
    ...
    const APIDeleteData = async (data) => {
        ...
        if (!response.status) {
            ...
            successDiv.innerHTML = "Водитель успешно удален!";
        ...
        e.preventDefault();
           ...
           if(response.message == "Redirect"){
                ...
                setTimeout(async () => {
                    ...
                    successDiv.innerHTML = alertMsg('info', `<p>${response.message}</p> <p>Идентификационный номер водителя: <strong>${response.driverId}</strong></p>`)
    ...
    const loadingBtn = `<i class='bx bx-loader-alt bx-spin'></i>`;
    const loadedBtn = `Сохранить изменения`;
        ...
        updateDriverBtn.setAttribute('disabled', true)
           ...
           if(response.message == "Redirect"){
                ...
                window.scrollTo({ 
                    top: 0, 
                    behavior: "smooth" });
                document.querySelector('#pError').innerHTML = "Данные отрудника обновляются...";
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if(params.get("s")){
            document.querySelector('#pError').innerHTML = "Данные водителя успешно обновлены!";
            document.querySelector('#pError').style.color = 'green';
